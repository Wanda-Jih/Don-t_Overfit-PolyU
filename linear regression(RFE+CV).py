...
selector = RFE(estimator=log, n_features_to_select=21, step=1) #, 25, step=1
selector.fit(x_train,y_train)

#selector = RFECV(estimator=log, cv=KFold(n_splits=2, random_state=None), scoring='neg_mean_squared_error') 
#selector.fit(x_train,y_train)



#result
prediction = selector.predict(test)
prediction = pd.DataFrame(prediction)
prediction.index += 250
prediction.columns = ['target']
prediction.to_csv('C:/Users/user/Desktop/class/maching learning/project/group/Don-t_Overfit_PolyU/result/Logistic Regression(RFE+CV).csv', index_label='id', index=True)
